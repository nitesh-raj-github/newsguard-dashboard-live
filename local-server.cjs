...
http.createServer(async (req, res) => {
  ...
  try {
    ...
    const answer = await fetch('https://api.groq.com/openai/v1/chat/completions', { method: 'POST', headers: { Authorization: `Bearer ${process.env.GROQ_API_KEY}`, 'Content-Type': 'application/json' }, body: JSON.stringify({ model: 'openai/gpt-oss-20b', temperature: .2, max_tokens: 260, messages: [{ role: 'system', content: 'You are a responsible media-literacy assistant. Never present classifier output as a fact check.' }, { role: 'user', content: prompt }] }) });
